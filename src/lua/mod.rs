...
pub fn call_internal<T: ToString>(app: &App, name: T) -> Option<Result<String, hlua::LuaError>> {
    ...
    let mut on_tick: LuaFunction<_> = internal.get(name.to_string())?;
    Some(on_tick.call())
}
...
pub fn run_ui_update<'a, T: ToString>(app: &App, event: T) -> Vec<Span<'a>> {
    ...
    match run_event {
        Some(mut v) => {
            ...
            for (_, v) in table.iter::<i32, String>().flatten() {
...
pub fn get_prompt(app: &App) -> String{
    let mut lua = app.lua.borrow_mut();
    let mut internal: LuaTable<_> = lua.get("_internal").unwrap();
    let run_event: Option<LuaFunction<_>> = internal.get("run_event");
    ...
    match run_event {
        Some(mut v) => {
            // let mut p: String = v.call_with_args("prompt".to_string()).unwrap();
            let mut ret = String::new();
            let mut table: LuaTable<_> = v.call_with_args("prompt".to_string()).unwrap_or_else(|e| panic!("{}", e));
            for (_, v) in table.iter::<i32, String>().flatten() {
                // ground is tells wether we are coloring the background forground or both
                let ground: u8 = v[0..1].parse().unwrap();
                let r = u8::from_str_radix(&v[1..=2], 16).unwrap();
                let g = u8::from_str_radix(&v[3..=4], 16).unwrap();
                let b = u8::from_str_radix(&v[5..=6], 16).unwrap();
                if ground == 2 {
                    ...
                    let bb = u8::from_str_radix(&v[10..12], 16).unwrap();
                    let value = v[13..=v.len() - 1].to_string();
                    ret.push_str(format!("\x1b[38;2;{r};{g};{b};48;2;{br};{bg};{bb}m{}", value).as_str());
                } else if ground == 1 {
                    let value = v[7..=v.len() - 1].to_string();
                    ret.push_str(format!("\x1b[48;2;{r};{g};{b}m{}", value).as_str());
                } else if ground == 0 {
                    let value = v[7..=v.len() - 1].to_string();
                    ret.push_str(format!("\x1b[38;2;{r};{g};{b}m{}", value).as_str());
                }
            }
            // panic!("{:?}", ret);
            ret
        ...
        None => {
            String::from("this is the prompt")
        }
    }
}
