...
// use std::{io::{Write, BufReader, BufRead, self}, env, sync::{mpsc::channel, Arc}, thread};
//
// use crate::app::{reset_terminal, pane::PaneRef};
//
// use self::test2::{open_pty, PtySize, CommandBuilder, MasterPty, SlavePty};
//
pub mod test;
pub mod test2;
//
...
// }
//
// pub fn after_run_command(mut _cmd: String, pane_ref: PaneRef) {
...
// }
//
// pub fn run_command(mut cmd: String, pane_ref: PaneRef, pair: (&MasterPty, &SlavePty)) {
//     let path = cmd.split(' ').next().unwrap().to_string();
//
//     {
...
//     }
//
//     // the pty needs to be created in once for each pain.
//
